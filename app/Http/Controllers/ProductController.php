...
class ProductController extends Controller
{
    ...
    public function productData(Request $request)
    {
        ...
        if (!empty($products)) {
            foreach ($products as $key => $product) {
                ...
                $nestedData['options'] = '<div class="dropdown d-inline-block">
                ...
                if ($product->is_imei) {
                    $nestedData['options'] .= '<li>
                            <a href="' . route('serial.view', $product->id) . '" class="btn btn-link"><i class="fa fa-eyedropper"></i> ' . trans('View Serial Key') . '</a>
                        </li>';
                }
    ...
    public function store(Request $request)
    {
        ...
        if (isset($data['is_variant'])) {
            foreach ($data['variant_name'] as $key => $variant_name) {
                ...
                $lims_product_variant_data->qty = $request->qty;
    ...
    {
        $serials = '';
        if ($request->input('data') && $request->input('serial_key')) {
            $preLoadedproduct = $this->limsProductSearch($request);
            $serials = Serial::where('serials', $request->input('serial_key'))->first();
        ...
        return view('backend.product.print_barcode', compact('lims_product_list_without_variant', 'serials', 'lims_product_list_with_variant', 'preLoadedproduct'));
    }
