    ...
    public function index()
    {
        ...
        if (Auth::user()->role_id > 2 && $general_setting->staff_access == 'own') {
            ...
            $product_sold = DB::table('product_sales')
                ...
                ->groupBy('product_id')
                ->count();
            $total_sales = $product_sale_data->count();
        } else {
        ...
        // Item Wise Sell
        $categories = Category::with(['product.purchase', 'product.sales'])->get();
        $categorySales = [];
        foreach ($categories as $category) {
            $totalSales = $category->product->flatMap(function ($product) {
                return $product->sales;
            })->sum('total');
            $qtySales = $category->product->flatMap(function ($product) {
                return $product->sales;
            })->sum('qty');
            $qtyInventory = $category->product->sum('qty');
            $costInventory = $category->product->sum('cost');

            $qtyPurchase = $category->product->flatMap(function ($product) {
                return $product->purchase;
            })->sum('qty');

            $totalPurchase = $category->product->flatMap(function ($product) {
                return $product->purchase;
            })->sum('total');

            $categorySales[$category->name]['total_sales'] = $totalSales;
            $categorySales[$category->name]['qty_sales'] = $qtySales;
            $categorySales[$category->name]['qty_inventory'] = $qtyInventory;
            $categorySales[$category->name]['cost_inventory'] = $costInventory;
            $categorySales[$category->name]['total_purchase'] = $totalPurchase;
            $categorySales[$category->name]['qty_purchase'] = $qtyPurchase;
        }
        // dd($categorySales);
        ...
        DB::reconnect();
        return view('backend.index', compact('revenue', 'categorySales', 'total_sales', 'product_sold', 'purchase', 'expense', 'return', 'purchase_return', 'profit', 'payment_recieved', 'payment_sent', 'month', 'yearly_sale_amount', 'yearly_purchase_amount', 'recent_sale', 'recent_purchase', 'recent_quotation', 'recent_payment', 'best_selling_qty', 'yearly_best_selling_qty', 'yearly_best_selling_price', 'all_permission'));
    }
